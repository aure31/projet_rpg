...
class anim_sprite():
    ...
    def __init__(self,
        ...
        if hitbox == None:
            ...
            self.ratio = (1, 1)
            self.coord = p.Vector2(coord.x - self.width / 2,
                                   coord.y - self.height / 2)
        else:
            self.ratio = (hitbox.x / self.raw_img.get_height(),
                          hitbox.y / self.raw_img.get_height())
            self.rect = hitbox
    ...
    def next_frame(self):
        if self.fps_time <= 0 or (not get_time_millis() - self.last_time
                                  >= self.fps_time):
            return
    ...
    def set_pos(self, x, y):
        self.coord = p.Vector2(x, y)
        if self.raw_rect is None:
            print("none")
            self.coord = p.Vector2(x - self.height / 2, y - self.height / 2)
        else:
            self.coord = p.Vector2(x - self.width / 2, y - self.height / 2)
        self.raw_coord = p.Vector2(x, y)

    def set_scale(self, scale: int):
        self.img = p.transform.scale(self.raw_img,
                                     (self.raw_img.get_width() * scale,
                                      self.raw_img.get_height() * scale))
        if self.raw_rect is None:
...
class entity(anim_sprite):
    ...
    def __init__(self, img: p.Surface, coord: tuple, Scale: float, fps: int,
                 speed: float):
